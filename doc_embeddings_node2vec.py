...
if __name__ == '__main__':
    # Load the Node2Vec model from the corresponding dataset directory
    node2vec_model = Word2Vec.load(os.path.join(load_save_path, f'{prefix}_node2vec'))
    # node2vec_model = Word2Vec.load("../document-classification-using-graph-embeddings/node2vec_models/node2vec.model")
    ...
    for category in os.listdir(parsed_path):
        ...
        for file in os.listdir(category_path):
            ...
            with open(file_path, "r") as text_file:
                ...
                words_found_vectors = [node2vec_model.wv.get_vector(word) for word in words if word in node2vec_model.wv.key_to_index]
    ...
    # Save the CSV file for Node2Vec to the corresponding dataset directory
    df.to_csv(os.path.join(load_save_path, f'{prefix}_embeddings_node2vec.csv'), index=False)
