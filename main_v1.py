    ...
    if window_flag:
        if embe_flag:
            if vaz_flag:
                try:
                    ...
                    # print(testdict)
                    adjmat = CreateAdjMatrix_Vazirgiannis_implementation_wn(temp[0], filename, window_size, mydict, testdict)
                ...
                try:
                    ...
                    testdict = loadNodeEmbedict(filename1)
                    # print(testdict)
                    adjmat = CreateAdjMatrixFromInvIndexWithWindow_wn(temp[0], filename, window_size, per_window, dot_split, mydict, testdict)
                except MemoryError:
                    sizeof_err_matrix = sys.getsizeof(adjmat)
                    print(sizeof_err_matrix)
                    exit(-1)
        ...
        if embe_flag:
            try:
                ...
                testdict = loadNodeEmbedict(filename1)
                # print(testdict)
                adjmat = CreateAdjMatrixFromInvIndexWithSenParWindow_wn(temp[0], filename, window_size, par_window_size, dot_split, mydict, testdict)
            except MemoryError:
                sizeof_err_matrix = sys.getsizeof(adjmat)
                print(sizeof_err_matrix)
                exit(-1)
